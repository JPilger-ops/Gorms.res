      ...
      <div className="page-frame grid min-w-0 gap-4 py-4 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-6 lg:py-10">
        <aside className="glass-panel p-4 lg:sticky lg:top-6 lg:self-start">
          ...
          >
            {navItems.map((item) => (
              <Link
                className="shrink-0 whitespace-nowrap rounded-2xl px-3 py-2 text-sm font-semibold text-muted transition-colors hover:bg-surface hover:text-foreground lg:whitespace-normal"
                href={item.href}
                key={item.href}
              >
                {item.label}
